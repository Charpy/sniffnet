...
impl PacketsNotification {
    ...
    pub fn from(value: &str, existing: Option<Self>) -> Self {
        ...
        let new_threshold = if value.is_empty() {
            ...
            value.parse().ok().or(default.threshold)
        };
        Self {
            threshold: new_threshold,
            previous_threshold: new_threshold.unwrap_or(default.previous_threshold),
            ..default
        }
    }
}
...
impl BytesNotification {
    ...
    pub fn from(value: &str, existing: Option<Self>) -> Self {
        ...
        let mut byte_multiple_inserted = default.byte_multiple;
        let new_threshold = if value.is_empty() {
            None
        } else if !value.trim().chars().map(char::is_numeric).any(|x| !x) {
            // no multiple
            byte_multiple_inserted = ByteMultiple::B;
            Some(value.parse::<u64>().unwrap_or(default.previous_threshold))
        } else {
            // multiple
            ...
            } else if without_multiple.is_empty() || existing.is_none() {
                None
            } else {
                byte_multiple_inserted = default.byte_multiple;
                Some(default.previous_threshold)
            }
        };
        Self {
            threshold: new_threshold,
...
impl FavoriteNotification {
    ...
    /// Constructor when the notification is in use. Note that sound is used here for caching, although it won't actively be used.
...
mod tests {
    ...
    #[case("foob@r", BytesNotification::default())]
    ...
    #[case("8888", PacketsNotification { 
        ...
        threshold: Some(8888),
        ..PacketsNotification::default() })]
    #[case("420 m", PacketsNotification::default())]
    #[case("foob@r", PacketsNotification::default())]
